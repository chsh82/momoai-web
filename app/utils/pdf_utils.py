...
def generate_teacher_manual_pdf():
    ...
    # ── 2. 앱 설치 및 알림 ────────────────────────────────────
    add_section('2', '앱 설치 및 알림', [
        ('body', '[홈 화면에 앱 추가하기 (PWA)]'),
        ('body', '앱스토어 없이 스마트폰 홈 화면에 아이콘을 추가해 앱처럼 사용할 수 있습니다.'),
        ('numbered', 'iPhone (iOS): Safari에서 momoai.kr 접속 → 하단 공유 버튼(□↑) 탭 → "홈 화면에 추가" 선택 → 알림 허용'),
        ('numbered', 'Android: Chrome에서 momoai.kr 접속 → 화면 하단 "홈 화면에 추가" 배너 탭 → 설치 후 알림 허용'),
        ('note', 'Push 알림은 Safari 브라우저가 아닌 홈 화면 앱(PWA)으로 실행할 때만 작동합니다. (iOS 16.4 이상)'),
        ('space', ''),
        ('body', '[강사에게 전달되는 알림 종류]'),
        ('bullet', '에세이 새 제출 알림: 담당 학생(또는 학부모 대리)이 에세이를 제출했을 때'),
        ('bullet', '내부 메세지 수신: 관리자 또는 학부모로부터 메세지를 받았을 때'),
        ('bullet', '문의 게시판 알림: 학부모/학생의 문의 글이 등록됐을 때'),
        ('space', ''),
        ('body', '[강사 행동 시 학생·학부모에게 자동 발송되는 알림]'),
        ('bullet', '결석/지각 처리 → 학부모 즉시 알림 (NEW)'),
        ('bullet', '수업 1시간 전 → 학생·학부모 자동 리마인더 (NEW)'),
        ('bullet', '주간 평가 저장 → 학부모 즉시 알림 (NEW)'),
        ('note', '알림은 상단 사이드바의 알림 메뉴에서도 언제든지 확인할 수 있습니다.'),
    ])

    # ── 3. 수업 관리 ─────────────────────────────────────────
    story.append(Paragraph("3. 수업 관리", section_style))
    ...
    # ── 7. 수업 공지 및 메세지 ───────────────────────────────
    story.append(Paragraph("7. 수업 공지 및 메세지", section_style))
    ...
    # ── 10. 게시판 ────────────────────────────────────────────
    story.append(Paragraph("10. 게시판", section_style))
    ...
    # ── 11. 학생 평가 ────────────────────────────────────────
    story.append(Paragraph("11. 학생 평가", section_style))
    ...
    # ── 12. FAQ ─────────────────────────────────────────────
    story.append(Paragraph("12. 자주 묻는 질문 (FAQ)", section_style))
...
def generate_student_manual_pdf():
    ...
    add_section('2', '앱 설치 및 알림', [
        ...
        ('space', ''),
        ('body', '[받게 되는 알림 종류]'),
        ('bullet', '수업 1시간 전 리마인더: 수업 시작 1시간 전에 자동으로 알림 발송 (NEW)'),
        ('bullet', '첨삭 완료 알림: 강사가 내 에세이 첨삭을 완료했을 때'),
        ('bullet', '과제 등록 알림: 강사가 새 과제를 출제했을 때'),
        ('bullet', '수업 공지 알림: 강사가 수업 공지를 등록했을 때'),
        ('bullet', '보강수업 결과 알림: 보강수업 신청이 승인/거절됐을 때'),
        ('note', '알림은 상단 사이드바의 알림 메뉴에서도 언제든지 확인할 수 있습니다.'),
    ])

    # 3. 수업
    story.append(Paragraph("3. 수업 확인", section_style))
    ...
    # 7. 교재/동영상
    story.append(Paragraph("7. 학습 교재 및 동영상", section_style))
    ...
    # 10. 평가
    story.append(Paragraph("10. 평가 및 테스트", section_style))
...
def generate_parent_manual_pdf():
    ...
    add_section('2', '앱 설치 및 알림', [
        ...
        ('space', ''),
        ('body', '[받게 되는 알림 종류]'),
        ('bullet', '결석/지각 즉시 알림: 자녀가 결석 또는 지각 처리됐을 때 즉시 발송 (NEW)'),
        ('bullet', '수업 1시간 전 리마인더: 자녀 수업 시작 1시간 전에 자동 발송 (NEW)'),
        ('bullet', '주간 평가 등록 알림: 강사가 자녀의 주간 평가를 입력했을 때 (NEW)'),
        ('bullet', '자녀 연결 결과 알림: 자녀 연결 신청이 승인/거절됐을 때'),
        ('bullet', '첨삭 완료 알림: 자녀의 에세이 첨삭이 완료됐을 때'),
        ('bullet', '수업 공지 알림: 강사가 수업 공지/메세지를 발송했을 때'),
        ('bullet', '보강수업 결과 알림: 보강수업 신청이 승인/거절됐을 때'),
        ('bullet', '내부 메세지 수신: 관리자/강사로부터 메세지를 받았을 때'),
        ('note', '알림은 상단 사이드바의 알림 메뉴에서도 언제든지 확인할 수 있습니다.'),
    ])
    ...
    add_section('5', '과제 및 첨삭 확인', [
        ...
        ('space', ''),
        ('body', '[자녀 대신 글 제출하기]'),
        ('numbered', '1) 사이드바 학습 현황 → 글 제출하기를 클릭합니다.'),
        ('numbered', '2) 자녀를 선택하고 담당 강사를 지정합니다.'),
        ('numbered', '3) 글 제목과 본문을 입력하거나 파일을 첨부합니다.'),
        ('numbered', '4) 제출하기 버튼을 누르면 담당 강사에게 알림이 전송됩니다.'),
    ])
    ...
    # 7. 평가
    story.append(Paragraph("7. 평가 정보", section_style))
    ...
    # 10. 게시판
    story.append(Paragraph("10. 게시판 및 피드백", section_style))
...
def generate_admin_manual_pdf():
    ...
    add_section('2', '학생 관리', [
        ...
        ('numbered', '3) 학생 상태 관리: 학생 상세 페이지에서 등록 / 휴원 / 퇴원 상태를 전환할 수 있습니다.'),
        ('numbered', '4) 학생 삭제: 상세 페이지 → 삭제 버튼 (모든 관련 데이터 함께 삭제)'),
        ('note', '퇴원 처리 후 7일간은 포털 이용 가능, 7일 경과 후 기능 차단됩니다.'),
    ])
    ...
    add_section('4', '결제 관리', [
        ('body', '사이드바 → 결제 관리에서 월별, 학생별 수납 내역을 확인합니다.'),
    ...
    add_section('10', '앱 설치 및 알림', [
        ('body', '[홈 화면에 앱 추가하기 (PWA)]'),
        ('body', '앱스토어 없이 스마트폰 홈 화면에 아이콘을 추가해 앱처럼 사용할 수 있습니다.'),
        ('numbered', 'iPhone (iOS): Safari에서 momoai.kr 접속 → 하단 공유 버튼(□↑) 탭 → "홈 화면에 추가" 선택 → 알림 허용'),
        ('numbered', 'Android: Chrome에서 momoai.kr 접속 → 화면 하단 "홈 화면에 추가" 배너 탭 → 설치 후 알림 허용'),
        ('note', 'Push 알림은 홈 화면 앱(PWA)으로 실행할 때만 작동합니다. (iOS 16.4 이상)'),
        ('space', ''),
        ('body', '[관리자 수신 알림]'),
        ('bullet', '학부모 자녀 연결 신청: 학부모가 자녀 연결을 신청했을 때'),
        ('bullet', '보강수업 신청: 학생/학부모가 보강수업을 신청했을 때'),
        ('bullet', '문의 게시판 새 글: 학부모/학생이 문의를 등록했을 때'),
        ('bullet', '내부 메세지 수신: 다른 계정에서 메세지를 보냈을 때'),
        ('space', ''),
        ('body', '[시스템 자동 발송 알림 (관리자 조작 없이 자동)]'),
        ('bullet', '결석/지각 즉시 알림: 강사가 결석/지각 처리 시 → 학부모에게 즉시 발송'),
        ('bullet', '수업 1시간 전 리마인더: 매 30분마다 자동 확인 → 학생·학부모에게 발송'),
        ('bullet', '주간 평가 등록 알림: 강사가 주간 평가 저장 시 → 학부모에게 즉시 발송'),
        ('note', '리마인더는 수업 세션의 날짜와 시작 시간이 입력된 경우에만 발송됩니다.'),
    ])

    add_section('11', '자주 묻는 질문', [
