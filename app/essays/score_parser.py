...
class ScoreParser:
    ...
    # Elementary model (초등) indicator labels
    ELEMENTARY_THINKING_TYPES = [
        '핵심찾기', '비교하기', '활용하기', '판단하기', '따져보기',
        '해결하기', '자료읽기', '내생각', '묶기'
    ]

    ELEMENTARY_INTEGRATED_INDICATORS = [
        '마무리', '짜임새', '알기쉬움', '문제발견', '정보',
        '주제맞춤', '여러쪽', '깊이', '빠짐없이'
    ]

    # Growth stage CSS class → (grade label, score midpoint)
    GROWTH_STAGE_MAP = {
        'g-fruit':  ('열매🍎', 95.0),
        'g-flower': ('꽃🌻', 85.0),
        'g-bud':    ('꽃봉오리🌸', 75.0),
    ...
    def parse_elementary_html(self, html_content: str) -> Dict:
        """
        초등(elementary) 모델 HTML에서 점수 추출.
        - 성장단계 CSS 클래스(g-fruit 등)로 final_grade / total_score 결정
        - SVG 레이더 차트 polygon 좌표에서 개별 지표 점수 역산
        """
        try:
            soup = BeautifulSoup(html_content, 'html.parser')

            # 1. 성장단계 추출 (div.ss 의 클래스 중 g-* 찾기)
            final_grade = None
            total_score_fallback = None
            ss_div = soup.find(class_='ss')
            if ss_div:
                for cls in ss_div.get('class', []):
                    if cls in self.GROWTH_STAGE_MAP:
                        final_grade, total_score_fallback = self.GROWTH_STAGE_MAP[cls]
                        break

            # 2. 개별 지표 점수 추출 (차트 카드 cc → SVG)
            thinking_scores = {}
            integrated_scores = {}

            for card in soup.find_all(class_='cc'):
                title_el = card.find(class_='ct')
                if not title_el:
                    continue
                ...
                if not svg:
                    continue
                if '사고유형' in title_text and not thinking_scores:
                    thinking_scores = self._extract_elem_scores_from_svg(
                        svg, self.ELEMENTARY_THINKING_TYPES)
                elif '통합' in title_text and not integrated_scores:
                    integrated_scores = self._extract_elem_scores_from_svg(
                        svg, self.ELEMENTARY_INTEGRATED_INDICATORS)

            # Fallback: SVG 순서로 추출
            if not thinking_scores or not integrated_scores:
                svgs = soup.find_all('svg')
                if len(svgs) >= 1 and not thinking_scores:
                    thinking_scores = self._extract_elem_scores_from_svg(
                        svgs[0], self.ELEMENTARY_THINKING_TYPES)
                if len(svgs) >= 2 and not integrated_scores:
                    integrated_scores = self._extract_elem_scores_from_svg(
                        svgs[1], self.ELEMENTARY_INTEGRATED_INDICATORS)

            # 3. total_score 계산: 0.50×(사고유형평균×10) + 0.50×(통합지표평균×10)
            total_score = None
            if thinking_scores or integrated_scores:
    ...
    def _extract_elem_scores_from_svg(self, svg, expected_labels: List[str]) -> Dict[str, float]:
        """
        Elementary SVG에서 점수 추출.
        1차: rl2(라벨) + rv(점수) 텍스트 매칭
        2차: polygon points 좌표 역산 (중심 130,115 / 반지름 85)
        """
        # 방법 1: rl2 라벨 + rv 점수 텍스트
        labels = []
        score_values = []
        for text in svg.find_all('text'):
            ...
            if 'rl2' in cls:
                labels.append(text_val)
            elif 'rv' in cls:
                try:
                    score_values.append(float(text_val))
                except ValueError:
                    pass

        if labels and score_values and len(labels) == len(score_values):
            return {label: score for label, score in zip(labels, score_values)}

        # 방법 2: polygon points 좌표 역산
        polygon = None
        for poly in svg.find_all('polygon'):
            poly_cls = poly.get('class', [])
            if 'rf' in poly_cls:
                polygon = poly
                break
        ...
        if len(points) != len(expected_labels):
            return {}

        cx, cy, r = 130.0, 115.0, 85.0
        scores = {}
        for i, (x, y) in enumerate(points):
            dist = math.sqrt((x - cx) ** 2 + (y - cy) ** 2)
            score = round(min(10.0, max(0.0, (dist / r) * 10.0)), 2)
            scores[expected_labels[i]] = score
