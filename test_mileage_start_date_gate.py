#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""마일리지 적립 시작일(2026-09-01, KST) 게이트 확인 스크립트

app/services/mileage_rules.py의 MILEAGE_START_DATE 하나만 참조 지점을
바꾸면 게이트를 통째로 걷어낼 수 있는지 확인하는 게 목적이다.
확인 항목:
  1) 8월 31일(KST) 활동 -> award_points()가 예외 없이 None 반환, 행 생성 안 됨
  2) 9월 1일(KST) 활동 -> 정상 적립
  3) UTC 기준 8/31이지만 KST로 환산하면 9/1인 경계 시각 -> 정상 적립
     (게이트가 KST 날짜로 판정한다는 것을 확인 - UTC로만 봤으면 걸렸을 케이스)
  4) 8월 31일 시작 주(=9/3이 속한 주) 배치 -> 건너뜀(빈 리스트, 로그만)
  5) 9월 7일 시작 주 배치 -> 정상 집계
...
with app.app_context():
    ...
    print("\n[1] award_points() 게이트 - 8월 31일(KST) 활동")
    ev = msvc.award_points(
        student_a, 'RW01', 'essay', f'gate-0831-{uuid.uuid4().hex[:8]}',
        occurred_at=datetime(2026, 8, 31, 0, 0, 0),  # UTC 00:00 -> KST 09:00, 같은 날짜
    )
    ...
    caught = False
    try:
        msvc.award_points(student_a, 'RW01', 'essay', f'gate-0831b-{uuid.uuid4().hex[:8]}',
                          occurred_at=datetime(2026, 8, 31, 12, 0, 0))
    except Exception:
        caught = True
    check("try/except로 감싸도 예외가 발생하지 않음", caught is False)

    print("\n[2] award_points() 게이트 - 9월 1일(KST) 활동")
    ev2 = msvc.award_points(
        student_a, 'RW01', 'essay', f'gate-0901-{uuid.uuid4().hex[:8]}',
        occurred_at=datetime(2026, 9, 1, 0, 0, 0),  # UTC 00:00 -> KST 09:00 09/01
    )
    check("정상 적립됨(PointEvent 반환)", ev2 is not None)
    check("점수가 그대로 지급됨(500점)", ev2 is not None and ev2.points == 500)

    print("\n[3] KST 환산 경계 - UTC로는 8/31이지만 KST로는 9/1인 시각")
    ev3 = msvc.award_points(
        student_a, 'QZ01', 'quiz_session', f'gate-boundary-{uuid.uuid4().hex[:8]}',
        occurred_at=datetime(2026, 8, 31, 23, 0, 0),  # UTC 23:00 -> KST 다음날 08:00
    )
    check("UTC 날짜가 아니라 KST 날짜로 판정해 정상 적립됨", ev3 is not None)

    db.session.commit()

    print("\n[4] 주간 출석(AT01) 배치 게이트 - 8월 31일 시작 주(9/3이 속한 주)")
    # 정규 수업 세션을 8/31~9/6 주간에 하나 만들고 전부 출석 처리 - 게이트가
    # 없다면 정상 지급될 상황을 만들어서, 게이트가 실제로 막는지 확인한다.
    ...
    session_0831 = CourseSession(course_id=course.course_id, session_number=1,
    ...
    attendance = Attendance(session_id=session_0831.session_id, student_id=student_b,
                            enrollment_id=enrollment.enrollment_id, status='present')
    db.session.add(attendance)
    db.session.commit()

    results_before_start = batch.run_weekly_attendance_batch(monday=date(2026, 8, 31), dry_run=True)
    check("8월 31일 시작 주는 아무 결과 없이 건너뜀(빈 리스트)", results_before_start == [])
    check("실제로 지급되지 않음(dry_run=False로도 재확인)",
          batch.run_weekly_attendance_batch(monday=date(2026, 8, 31), dry_run=False) == [])
    check("point_events에 AT01 행이 생기지 않음",
          PointEvent.query.filter_by(student_id=student_b, activity_code='AT01').count() == 0)

    print("\n[5] 주간 출석(AT01) 배치 - 9월 7일 시작 주는 정상 집계")
    session_0907 = CourseSession(course_id=course.course_id, session_number=2,
                                 session_date=date(2026, 9, 7), status='scheduled')
    db.session.add(session_0907)
    db.session.flush()
    created_session_ids.append(session_0907.session_id)
    attendance2 = Attendance(session_id=session_0907.session_id, student_id=student_b,
                             enrollment_id=enrollment.enrollment_id, status='present')
    db.session.add(attendance2)
    db.session.commit()

    results_after_start = batch.run_weekly_attendance_batch(monday=date(2026, 9, 7), dry_run=True)
    entry_b = next((r for r in results_after_start if r['student_id'] == student_b), None)
    check("9월 7일 시작 주는 정상 집계됨(대상 학생 존재)", entry_b is not None)
    ...
    # 시각)로 게이트를 검사한다. 이 스크립트를 실제로 돌리는 오늘 날짜는
    # 아직 2026-09-01 이전이라 배치의 주간 게이트를 통과해도 award_points()
    # 자체 게이트에 다시 걸린다 - 이는 배치가 잘못된 게 아니라 "실제로 이
    # 배치가 9월 이후에 돌아간다"는 전제를 오늘 이 스크립트 안에서 재현하려면
    ...
    with patch('app.services.mileage_service.datetime') as mock_dt:
        mock_dt.utcnow.return_value = datetime(2026, 9, 10)
        real_results = batch.run_weekly_attendance_batch(monday=date(2026, 9, 7), dry_run=False)
    db.session.commit()
    at01_event = PointEvent.query.filter_by(student_id=student_b, activity_code='AT01').first()
    check("9월 이후 실행 시(시각 재현) AT01이 실제로 지급됨", at01_event is not None)
