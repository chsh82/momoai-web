# -*- coding: utf-8 -*-
"""
모모의 책장 연간 커리큘럼 엑셀을 curriculum_weeks + books 테이블로 임포트.
...
def _find_or_create_book(title_index: dict, title: str, author: str | None, publisher: str | None,
                          grade_code: str, stats: dict) -> Book:
...
def import_curriculum(path: str, dry_run: bool = False):
    ...
    for r in rows:
        ...
        if not r['is_holiday'] and r['title']:
            ...
            if clean_title:
                book = _find_or_create_book(title_index, clean_title, r['author'], r['publisher'], r['grade'], stats)
        else:
...
if __name__ == '__main__':
    ...
    with app.app_context():
        import_curriculum(args.excel_path, dry_run=args.dry_run)
